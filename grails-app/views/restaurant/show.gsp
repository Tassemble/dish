	...
	<body>
		...
		<div id="show-restaurant" class="content scaffold-show" role="main">
			...
			<ol class="property-list restaurant">
				...
				<g:if test="${restaurantInstance?.manyDishes}">
				<li class="fieldcontain">
					<span id="manyDishes-label" class="property-label"><g:message code="restaurant.manyDishes.label" default="Many Dishes" /></span>
					
						<g:each in="${restaurantInstance.manyDishes}" var="m">
						<span class="property-value" aria-labelledby="manyDishes-label"><g:link controller="dish" action="show" id="${m.id}">${m?.encodeAsHTML()}</g:link></span>
						</g:each>
